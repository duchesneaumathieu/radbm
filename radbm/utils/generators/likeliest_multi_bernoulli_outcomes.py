...
def likeliest_multi_bernoulli_outcomes(log_probs0, log_probs1, yield_stats=False):
    """
    ...
    for data in smallest_subset_sums(flipped_diff, yield_stats=True):
        outcome = most_probable_outcome.copy()
        outcome[data[0]] ^= True
        yield ((outcome,) + data[1:]) if yield_stats else outcome