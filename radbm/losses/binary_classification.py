...
class FbetaLoss(object):
    ...
    Parameters
    ----------
    log2_beta : float or tuple (positive)
        The parameter that control the importance of the recall over
        the precision. If a tuple of four float (n0, n1, log_beta0, log_beta1),
        __call__ takes an additional parameter n which determines the log_beta linearly
        between log_beta0 and log_beta1.
    ...
    """
    def __init__(self, log2_beta, prob_y1, naive=False, estimator_sharing=True):
        self.log2_beta = (lambda n: Ramp(*log2_beta)(n)) if isinstance(log2_beta, (tuple,list)) else (lambda n: log2_beta)
        self.prob_y1 = prob_y1
        self.naive = naive
        self.estimator_sharing = estimator_sharing
    
    def __call__(self, tp_log_probs, fp_log_probs, step=0):
        """
        Parameters
        ----------
        ...
        n : float (positive) (optional)
            Controls the ramping of the log_beta. (default 0)
        """
        ...
        beta = 2**self.log2_beta(step)
        log_prob_y1 = float(np.log(self.prob_y1))
        log_prob_y0 = float(np.log(1-self.prob_y1))
        log_c1 = float(np.log(self.prob_y1*beta**2))
        log_c3 = float(np.log(1 + 1/beta**2))
        log_recall = torch_lme(t2_tp_log_probs)
        log_fallout = torch_lme(fp_log_probs)
        log_t = torch_logsumexp(
            log_prob_y1 + log_recall,          
            log_prob_y0 + log_fallout)
        term2 = torch.nn.Softplus()(log_t - log_c1)
        log_fbeta_score_estimator =  term1 - term2 + log_c3
...
class BCELoss(object):
    ...
    Parameters
    ----------
    log2_lambda : float (in [-inf, 0]) or tuple (of size 4)
        It controls the weighting of true positive. over the true negatives.
    """
    def __init__(self, log2_lambda=-1.):
        self.log2_lambda = (lambda n: Ramp(*log2_lambda)(n)) if isinstance(log2_lambda, (tuple,list)) else (lambda n: log2_lambda)
    
    def __call__(self, tp_log_probs, tn_log_probs, step=0):
        """
        Parameters
        ----------
        tp_log_probs : torch.tensor (dtype: float)
        ...
        lmda = 2**self.log2_lambda(step)
        return -lmda*tp_log_probs.mean() - (1-lmda)*tn_log_probs.mean()