...
class FakeEngine(object):
    def __init__(self, cands, costs):
        self.cands = iter(cands)
...
class TestTCR(unittest.TestCase):
    def test_costs_at_k(self):
        ...
        #test 1
        candidates = FakeEngine(cands, costs)
        relevants = {0, 4, 5, 6, 7}
        ...
        uck, eck = costs_at_k(FakeEngine([], []), relevants, N=12)
        ...
        with self.assertRaises(ValueError):
            uck, eck = costs_at_k(FakeEngine(cands, costs), relevants)
        
        with self.assertRaises(RuntimeError):
            uck, eck = costs_at_k(FakeEngine(cands, costs), relevants, N=13)
        uck, eck = costs_at_k(FakeEngine(cands, costs), relevants, N=15)
        expected_uck = torch.tensor([0+1*3/2, 3+1*4/3, 3+2*4/3, 6+1*1/1, 7+1*3/2, 12+1*(15-12+1)/3, 12+2*(15-12+1)/3])
        expected_eck = torch.tensor([2., 8., 8., 11., 15., 25., 25.])
        self.assertTrue(torch.allclose(uck, expected_uck))
        self.assertTrue(torch.allclose(eck, expected_eck))
    ...
    def test_total_cost_at_k(self):
        ...
        costs = [    2.,  6.,        8.,    9., 11.,    15.,         25.]
        relevants = {0, 4, 5, 6, 7}
        total_cost_at_k(FakeEngine(cands, costs), relevants) #just make sure it runs
    ...
    def test_total_cost_ratio(self):
        ...
        for N in range(14, 20):
            ...
            tcr = total_cost_ratio(FakeEngine(cands, costs), relevants, N)
